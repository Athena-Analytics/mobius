...
import pandas as pd
import pendulum
from airflow.decorators import dag, setup, task
...
def demo_postgres_connection():
    ...
    @task()
    def write_using_insert(
        pg_hook: PostgresHook, df: pd.DataFrame, table_schema: str, table_name: str
    ):
    ...
    @task()
    def read_using_sql(
        pg_hook: PostgresHook, sql: str, sql_params: dict | None = None
    ) -> pd.DataFrame:
        engine = pg_hook.get_sqlalchemy_engine()
        return pd.read_sql(sql, con=engine, params=sql_params)

    @task()
    def read_using_sql_file(
        pg_hook: PostgresHook, sql: str, sql_params: dict | None = None
    ) -> pd.DataFrame:
        if sql.endswith(".sql"):
    ...
    @setup
    def truncate_table(
        pg_hook: PostgresHook, table_schema: str, table_name: str
    ) -> str:
        from contextlib import closing
    ...
    # test read
    read_using_sql(
        pg_hook, "SELECT * FROM dim.dim_date WHERE dt = %(dt)s", {"dt": "2024-01-01"}
    )
    read_using_sql_file(
        pg_hook,
        "/opt/airflow/include/sql/test.sql",
        {"symbol": "TQQQ", "week_start": "2024-01-01"},
    )
